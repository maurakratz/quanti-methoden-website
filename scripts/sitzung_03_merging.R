...
# SITZUNG 03 Data merging & Quarto                      -----------
...
btw_2025_strukturdaten %>%
  distinct(wahlkreis_nr, wahlkreis_name)
   ...
   # das gesamte Bundesgebiet (Wahlkreise gibt es ja nur 299)

# Insgesamt ist das Problem, dass zwar beide Daten zu a) Wahlkreisen, b) Bundes-
   # ländern und c) dem Bundesgebiet enthalten, diese Daten aber in Strukturdaten
   # in einer einzigen Variable stecken, im Ergebnisdatensatz aber in drei
...
btw2025_strukturdaten_merge <- btw2025_strukturdaten %>%
  dplyr::mutate(
    gebietsart = dplyr::case_when(
      wahlkreis_nr == 999 ~ "Bund",
      wahlkreis_nr >= 901 ~ "Land",
      TRUE ~ "Wahlkreis"
    ),
    wahlkreis_nr = dplyr::case_when(
      wahlkreis_nr == 999 ~ 99,
      wahlkreis_nr == 901 ~ 1,
      wahlkreis_nr == 902 ~ 2,
      wahlkreis_nr == 903 ~ 3,
...
# Merge durchführen
btw_2025 <- btw_2025_ergebnisse %>%
  dplyr::left_join(btw2025_strukturdaten_merge,
